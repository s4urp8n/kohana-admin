...
class Kohana_Text
{
    ...
    public static function limit_words($str, $limit = 100, $end_char = null)
    {
        ...
        return rtrim($matches[0]) . ((strlen($matches[0]) === strlen($str))
            ? ''
            : $end_char);
    }
    ...
    public static function auto_link_urls($text)
    {
        ...
        $text = preg_replace_callback(
            '~\b(?<!href="|">)(?:ht|f)tps?://[^<\s]+(?:/|\b)~i', 'Text::_auto_link_urls_callback1', $text
        );
        
        // Find and replace all naked www.links.com (without http://)
        return preg_replace_callback(
            '~\b(?<!://|">)www(?:\.[a-z0-9][-a-z0-9]*+)+\.[a-z]{2,6}[^<\s]*\b~i', 'Text::_auto_link_urls_callback2',
            $text
        );
    }
    ...
    public static function auto_link_emails($text)
    {
        ...
        return preg_replace_callback(
            ...
            'Text::_auto_link_emails_callback', $text
        );
    }
        ...
        if ($value === 'browser' OR $value == 'version')
        {
            ...
            foreach ($browsers as $search => $name)
            {
                if (stripos($agent, $search) !== false)
                {
                    ...
                    if (preg_match(
                        '#' . preg_quote($search) . '[^0-9.]*+([0-9.][0-9.a-z]*)#i', Request::$user_agent, $matches
                    ))
                    {
