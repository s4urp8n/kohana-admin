...
class Kohana_Request implements HTTP_Request
{
    ...
    {
        $client_params = is_array($client_params)
            ? $client_params
            : [];
    ...
    public static function factory($uri = true, $client_params = [], $allow_external = true, $injected_routes = [])
    {
        ...
        if (!Request::$initial)
        {
            ...
            if (isset($_SERVER['HTTP_X_FORWARDED_FOR']) AND isset($_SERVER['REMOTE_ADDR']) AND in_array(
                    $_SERVER['REMOTE_ADDR'], Request::$trusted_proxies
                )
            )
            {
            ...
            elseif (isset($_SERVER['HTTP_CLIENT_IP']) AND isset($_SERVER['REMOTE_ADDR']) AND in_array(
                    $_SERVER['REMOTE_ADDR'], Request::$trusted_proxies
                )
            )
            {
    ...
    public function headers($key = null, $value = null)
    {
        ...
        elseif ($value === null)
        {
            ...
            return ($this->_header->offsetExists($key))
                ? $this->_header->offsetGet($key)
                : null;
        }
    ...
    public function execute()
    {
        ...
        {
            return HTTP_Exception::factory(
                404, 'Unable to find a route to match the URI: :uri', [
                       ':uri' => $this->_uri,
                   ]
            )
                                 ->request($this)
