    ...
    protected static function _dump(& $var, $length = 128, $limit = 10, $level = 0)
    {
        ...
        elseif (is_resource($var))
        {
            if (($type = get_resource_type($var)) === 'stream' AND $meta = stream_get_meta_data($var))
            {
                ...
                if (isset($meta['uri']))
                {
                    ...
                    return '<small>resource</small><span>(' . $type . ')</span> ' . htmlspecialchars(
                        $file, ENT_NOQUOTES, Kohana::$charset
                    );
                }
            }
            else
        ...
        elseif (is_string($var))
        {
            ...
            if (UTF8::strlen($var) > $length)
            {
                ...
                $str =
                    htmlspecialchars(UTF8::substr($var, 0, $length), ENT_NOQUOTES, Kohana::$charset) . '&nbsp;&hellip;';
            }
            else
            {
                // Encode the string
        ...
        {
            return '<small>' . gettype($var) . '</small> ' . htmlspecialchars(
                print_r($var, true), ENT_NOQUOTES, Kohana::$charset
            );
        }
    }
