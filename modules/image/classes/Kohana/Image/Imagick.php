...
class Kohana_Image_Imagick extends Image
{
    ...
    protected function _do_reflection($height, $opacity, $fade_in)
    {
        ...
        $fade = new Imagick;
        $fade->newPseudoImage(
            $reflection->getImageWidth(), $reflection->getImageHeight(), vsprintf('gradient:%s-%s', $direction)
        );
        ...
        if ($image->compositeImage($this->im, Imagick::COMPOSITE_SRC, 0, 0) AND $image->compositeImage(
                $reflection, Imagick::COMPOSITE_OVER, 0, $this->height
            )
        )
        {
        ...
        switch ($format)
        {
            ...
            default:
                throw new Kohana_Exception(
                    'Installed ImageMagick does not support :type images', [':type' => $extension]
                );
                break;
        }
