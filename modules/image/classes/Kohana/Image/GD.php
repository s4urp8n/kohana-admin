...
class Kohana_Image_GD extends Image
{
    ...
    public function __construct($file)
    {
        ...
        if (!isset($create) OR !function_exists($create))
        {
            throw new Kohana_Exception(
                'Installed GD does not support :type images', [':type' => image_type_to_extension($this->type, false)]
            );
        }
    ...
    protected function _do_resize($width, $height)
    {
        ...
        if ($width > ($this->width / 2) AND $height > ($this->height / 2))
        {
            ...
            if (imagecopyresized(
                $image, $this->_image, 0, 0, 0, 0, $pre_width, $pre_height, $this->width, $this->height
            ))
            {
    ...
    protected function _do_save($file, $quality)
    {
        ...
        $status = isset($quality)
            ? $save($this->_image, $file, $quality)
            : $save($this->_image, $file);
