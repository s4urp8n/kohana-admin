...
class Kohana_Flexiblemigrations
{
    ...
    {
        $this->_config = Kohana::$config->load('flexiblemigrations')
                                        ->as_array();
    }
    ...
    public function rollback()
    {
        ...
        if ($model->loaded())
        {
            try
            {
                ...
                {
                    $msg =
                        "Migration '" . $model->name . "' with hash: " . $model->hash . ' was succefully "rollbacked"';
                    $messages[] = array(0 => $msg);
                }
                else
                {
    ...
    public function generate_migration($migration_name)
    {
        try
        {
            ...
            file_put_contents($file, $view, LOCK_EX);
