...
class Drivers_Mysql extends Drivers_Driver
{
            ...
            if (is_string($key))
            {
                switch ($key)
                {
                    ...
                    case 'unsigned':
                        ...
                        break;
                    default:
                        throw new Kohana_Exception('migrations.bad_column :key', array(':key' => $key));
                }
    ...
    public function rename_column($table_name, $column_name, $new_column_name, $params)
    {
        ...
        $sql =
            "ALTER TABLE `$table_name` CHANGE `$column_name` " . $this->compile_column($new_column_name, $params, true);
    ...
    protected function get_column($table_name, $column_name)
    {
        ...
        if ($result->count() !== 1)
        {
            throw new Kohana_Exception(
                'migrations.column_not_found :col_name, :table_name',
                array(':col_name' => $column_name, ':table_name' => $table_name)
            );
        }
    ...
    public function add_index($table_name, $index_name, $columns, $index_type = 'normal')
    {
        switch ($index_type)
        {
            ...
            default:
                throw new Kohana_Exception(
                    'migrations.bad_index_type :index_type', array(':index_type' => $index_type)
                );
        }
