...
class Kohana_Pagination
{
    ...
    public function setup(array $config = [])
    {
        ...
        if ($this->current_page === null OR isset($config['current_page']) OR isset($config['total_items'])
        ...
        )
        {
            ...
            $this->current_first_item =
                (int)min((($this->current_page - 1) * $this->items_per_page) + 1, $this->total_items);
            $this->current_last_item =
                (int)min($this->current_first_item + $this->items_per_page - 1, $this->total_items);
            $this->previous_page = ($this->current_page > 1)
                ? $this->current_page - 1
                : false;
            $this->next_page = ($this->current_page < $this->total_pages)
                ? $this->current_page + 1
                : false;
            $this->first_page = ($this->current_page === 1)
                ? false
                : 1;
            $this->last_page = ($this->current_page >= $this->total_pages)
                ? false
                : $this->total_pages;
    ...
    public function url($page = 1)
    {
        ...
        switch ($this->config['current_page']['source'])
        {
            case 'query_string':
                ...
                return URL::site(
                    $this->_route->uri($this->_route_params) . $this->query(
            ...
            case 'route':
                ...
                return URL::site(
                    $this->_route->uri(
                        array_merge($this->_route_params, [$this->config['current_page']['key'] => $page])
                    ) . $this->query()
                );
        }
