...
class Controller_Admin extends Controller_Template
{
    ...
    public function check(
        array $allowes = ['isAllowed'],
        array $methodsExists = [],
        array $mandatoryRequestParams = [],
        array $mandatoryQueryParams = [],
    ...
    public static function goMenu()
    {
        if (Auth::instance()
            ...
            if (Auth::instance()
                ...
                Common::redirect(AdminHREF::getDefaultAdminRouteUri('/'));
            }
        ...
        if (!empty($_POST)) {
            ...
            if (!empty($login) && !empty($password)) {
                ...
                if ($this->auth->login($login, $password, $remember)) {
                    ...
                    $error = ___('Неправильное имя пользователя или пароль. Попробуйте ещё раз.');
                }
            } else {
                if (empty($login)) {
                    $error = ___('Пустое имя пользователя.');
                } else {
                    $error = ___('Пустой пароль');
                }
            }
        }
        ...
        if (Common::config(['admin', 'SignInSignUp'])) {
            ...
            if (!empty($_POST)) {
                ...
                if (!empty($email) && !empty($password) && !empty($captcha) && !empty($phone)) {
                    ...
                    if (!empty($exist->id)) {
                        $error = ___('Пользователь с таким email уже существует. Введите другой email.');
                    ...
                    if (empty($email)) {
                        $error = ___('Введите email.');
                    } elseif (empty($password)) {
                        $error = ___('Введите пароль.');
                    } else {
                        $error = ___('Введите проверочный код');
                    }
                }
            }
