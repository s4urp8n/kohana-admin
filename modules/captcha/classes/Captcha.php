...
abstract class Captcha
{
    ...
    public function __construct($group = null)
    {
        ...
        if ($group !== 'default')
        {
            ...
            if (!is_array(
                $default = Kohana::$config->load('captcha')
                                          ->get('default')
            )
            )
            {
                throw new Kohana_Exception(
                    'Captcha group not defined in :group configuration', [':group' => 'default']
                );
            }
        ...
        if (!empty($config['background']))
        {
            ...
            {
                throw new Kohana_Exception(
                    'The specified file, :file, was not found.', [':file' => Captcha::$config['background']]
                );
            }
        }
        ...
        if (!empty($config['fonts']))
        {
            ...
            foreach ($config['fonts'] as $font)
            {
                ...
                {
                    throw new Kohana_Exception(
                        'The specified file, :file, was not found.', [':file' => Captcha::$config['fontpath'] . $font]
                    );
                }
            }
        }
    ...
    public static function valid($response)
    {
        ...
        // Challenge result
        $result = (bool)(sha1(UTF8::strtoupper($response)) === Session::instance()
    ...
    public function update_response_session()
    {
        ...
        Session::instance()
               ->set('captcha_response', sha1(UTF8::strtoupper($this->response)));
    }
    ...
    public function image_create($background = null)
    {
        ...
        if (!empty($background))
        {
            ...
            if (imagesx($this->background_image) !== Captcha::$config['width'] or imagesy($this->background_image)
                                                                                  !== Captcha::$config['height']
            )
            {
                imagecopyresampled(
                    $this->image, $this->background_image, 0, 0, 0, 0, Captcha::$config['width'],
                    Captcha::$config['height'], imagesx($this->background_image), imagesy($this->background_image)
                );
            }
    ...
    public function image_render($html)
    {
        ...
        if ($html === true)
        {
            return '<img src="' . URL::site('captcha/' . Captcha::$config['group']) . '" width="'
                   . Captcha::$config['width'] . '" height="' . Captcha::$config['height']
