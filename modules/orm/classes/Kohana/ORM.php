...
class Kohana_ORM extends Model implements serializable
{
    ...
    protected function _initialize()
    {
        ...
        if (!$init = Arr::get(ORM::$_init_cache, $this->_object_name, false))
        {
            ...
            foreach ($this->_has_many as $alias => $details)
            {
                ...
                {
                    $defaults['model'] =
                        str_replace(' ', '_', ucwords(str_replace('_', ' ', Inflector::singular($alias))));
                }
    ...
    public function with($target_path)
    {
        ...
        $this->join([$target->_table_name, $target_path], 'LEFT')
    ...
    protected function _load_result($multiple = false)
    {
        ...
        if ($multiple === true)
        {
            ...
            $result = $this->_db_builder->as_object(get_class($this))
                                        ->execute($this->_db);
        ...
        else
        {
            ...
            $result = $this->_db_builder->as_assoc()
                                        ->execute($this->_db);
    ...
    {
        return (isset($this->_object[$column]) OR isset($this->_related[$column]) OR isset($this->_has_one[$column])
                OR isset($this->_belongs_to[$column]) OR isset($this->_has_many[$column]));
    }
    ...
    protected function run_filter($field, $value)
    {
        ...
        $wildcards = empty($filters[true])
            ? []
            : $filters[true];
        
        // Merge in the wildcards
        $filters = empty($filters[$field])
            ? $wildcards
            : array_merge($wildcards, $filters[$field]);
    ...
    public function update(Validation $validation = null)
    {
        ...
        {
            throw new Kohana_Exception(
                'Cannot update :model model because it is not loaded.', [':model' => $this->_object_name]
            );
        }
    ...
    public function create(Validation $validation = null)
    {
        ...
        {
            throw new Kohana_Exception(
                'Cannot create :model model because it is already loaded.', [':model' => $this->_object_name]
            );
        }
    ...
    public function count_relations($alias, $far_keys = null)
    {
        ...
        $far_keys = ($far_keys instanceof ORM)
            ? $far_keys->pk()
            : $far_keys;
